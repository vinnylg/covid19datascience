...
def read(path:str=join(dirname(__root__), 'tmp', 'srag.dbf')):
    ...
    srag_cases = pd.DataFrame(df, columns = ['nu_notific', 'dt_notific', 'nm_pacient', 'cs_sexo', 'dt_nasc',
                                        'nm_mae_pac', 'cs_raca', 'sg_uf', 'co_mun_res', 'sg_uf_not',
                                        'co_mun_not', 'classi_fin', 'evolucao', 'dt_evolucao'])
    srag_cases = srag_cases.rename(columns={'nu_notific':'nu_not','dt_notific':'dt_not', 'nm_pacient':'name',
                                       ...
                                       'co_mun_not':'ibge_not', 'classi_fin':'classification',
                                        'evolucao':'evolution', 'dt_evolucao':'dt_evo'})
    ...
    srag_cases = pd.DataFrame(srag_cases, columns = ['nu_not', 'dt_not', 'name', 'sex', 'dt_nasc','mom', 'race',
